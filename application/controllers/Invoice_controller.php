<?php
    ...
    class Invoice_controller extends Main_controller
    {
        ...
        private function getData_invoice_add_view()
        {
            ...
            // Set the predicted number for the next invoice
            {
                $lastNo = $this->Invoice_model->getLastNumber();
                $lastNo > 9 or $lastNo = "0" . $lastNo;
                
                $data[__DB_INVOICES_INVOICENUMBER__] = date("Y") . "_" . date("m") . "_" . $lastNo;
            }
            
            // Add all customers' descriptions 
            {
                $customers = $this->Customer_model->get();
                foreach ($customers as $customer) {
                    $data[__DB_CUSTOMERS__][$customer[__DB_CUSTOMERS_CUSTOMERID__]] = $customer[__DB_CUSTOMERS_NAME__] . " - " . $this->fetch_customer_address($customer);
                }
            }
        ...
        *                ["Currencies"]
        */
        private function getData_invoice_edit_view($invoiceId, $copyMode = false){
            $this->load->model("Invoice_model");
            $this->load->model("Customer_model");
            
            // Get data about the desired invoice
            $data = $this->Invoice_model->get($invoiceId);
            ...
            // Add list of all languages and currencies
            $data["Languages"] = $this->getLanguages();
            $data["Currencies"] = $this->getCurrencies();
            
            // If it isn't editing but copying the content, alter the data to prevent from conflicts.
            if ($copyMode == true) {
        ...
        /*
         * Adds an invoice using post data, then redirects to the invoices list
         */
        ...
        /*
         * Edits the invoice using post data, then redirects to the invoices list
         */
        ...
        /*
         * Shows the invoice in the pdf format.
         * !WARNING! The title is missleading, the method doesn't download the pdf invoice.
         */
        ...
        /*
         * Controls the view flow when user wants to list all invoices
         */
        ...
        /*
         * Controls the view flow when user wants to add an invoice
         */
        public function invoice_add_view()
        {
            $data = array();
            $data["fromController"] = $this->getData_invoice_add_view();
        ...
        /*
         * Controls the view flow when user wants to edit an invoice
         */
        public function invoice_edit_view($invoiceId, $copyMode = false)
        {
            $data = array();
            $data["fromController"] = $this->getData_invoice_edit_view($invoiceId, $copyMode);
        ...
        /*
         * Controls the view flow when user wants to view invoice
         */
